...
variable "autoscaling_groups" {
  ...
  validation {
    condition = can(
      regex(
        "([0-9])",
        var.autoscaling_groups.max_size
      )
    )
    error_message = "The max size must be a string number."
  }
  validation {
    condition = can(
      regex(
        "([0-9])",
        var.autoscaling_groups.min_size
      )
    )
    error_message = "The min size must be a string number."
  }
}
